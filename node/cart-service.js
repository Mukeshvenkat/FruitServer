...
class CartService {
    ...
    */
    static async purchase(request, response, next) {
        let fruitBill = request.body;
        ...
        if (purchaseStatus) {
            response.staus(200).json("Purchase Succesfully completed");
        } else {
            response.staus(200).json("Purchase UnSuccesfull");
        }
    }
}
